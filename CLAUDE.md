...
| 9 | D4 | `evidenced_by` may be created by `evidence` (Concept -> WebEvidence: this concept has retrieved evidence) or `compose` (LessonBlock -> SourceSpan\|WebEvidence: this generated sentence cites this span) | Both are "X is evidenced by Y", just at different pipeline stages; §4's table predates D4 and named only `evidence`. `Graph.add` now accepts a set of owning passes per edge kind |
