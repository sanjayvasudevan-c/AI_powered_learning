...
| D4 | 2026-09-11 | `d868f7e` + _pending push_ | dossier (byte-stable grounding assembly), compose (per-slot generation, empty-dossier refusal enforced in code), verify (critic sees only cited spans, repair-then-drop, quarantine), compute (SymPy-in-subprocess), origin linter (I2). `coursec build` now runs the full pipeline through verify. |
